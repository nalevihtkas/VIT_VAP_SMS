...
proc require_tempus_eco {} {
    if {![llength [info commands eco_opt_design]]} {
        puts "ERROR: eco_opt_design is unavailable in this Tempus session."
        puts "Try: puts [info commands *eco*]"
        error "Tempus ECO command eco_opt_design not available"
    }
}

proc run_setup_eco {} {
    require_tempus_eco
    puts "\nINFO: Running automatic SETUP ECO using eco_opt_design -setup"
    if {[catch {eco_opt_design -setup} msg]} {
        puts "\nERROR: eco_opt_design -setup was rejected."
        puts "Cadence message: $msg"
        catch {help eco_opt_design} H
        puts "\nInstalled help for eco_opt_design:\n$H"
        error "Setup ECO syntax must be adjusted for this exact Tempus build."
    }
}

proc run_hold_eco {} {
    require_tempus_eco
    puts "\nINFO: Running automatic HOLD ECO using eco_opt_design -hold"
    if {[catch {eco_opt_design -hold} msg]} {
        puts "\nERROR: eco_opt_design -hold was rejected."
        puts "Cadence message: $msg"
        catch {help eco_opt_design} H
        puts "\nInstalled help for eco_opt_design:\n$H"
        error "Hold ECO syntax must be adjusted for this exact Tempus build."
    }
}
