...
run_case $ROOT smart_sensor_datapath_opt optimized optimized.sdc {
set RS $ROOT/reports/optimized/retiming_status.txt
if {[catch {set_db design:$TOP .retime true} RETMSG]} {
    set F [open $RS w]; puts $F "COMMAND_REJECTED: $RETMSG"; close $F
    puts "WARNING: Retiming control not accepted: $RETMSG"
} else {
    set F [open $RS w]; puts $F "COMMAND_ACCEPTED"; close $F
}
} {}
