...
proc include_these_buttons {} {
  ...
  add_frame   "Control"
  add_button  "Create Library"  {vlib work; vmap work work}
  add_button  "Compile"         {  vlog Adder.v
                                   vlog Adder_tb.v }                                    
  add_button  "Load Simulation" {vsim -voptargs=+acc work.Adder_tb}
  add_button  "Wave Window"     {add wave -r /*}
  add_button  "Quit"            {quit -force}

  add_frame   "Simulate"
  add_button  "Run 10"   {run 10}
  add_button  "Run 100"  {run 100}
  add_button  "Run 1000" {run}
...
proc add_button  {title command} {
  global buttons
  button $buttons(frame).b$buttons(widget_count) -text $title -font $buttons(font) -command "puts \"$command\""
